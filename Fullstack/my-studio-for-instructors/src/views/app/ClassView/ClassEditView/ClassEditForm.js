...
import { updateClass, uploadImages } from 'src/slices/class';
...
const ClassCreateForm = ({ className, classInstance, ...rest }) => {
  ...
  const { isUploading, imageLinks } = useSelector(state => state.classes);
  console.log('Class for edit', classInstance);
  ...
  useEffect(() => {
    const _imgs = classInstance.images;
    if (_imgs !== null && _imgs.length !== 0) {
      const photos = [];
      _imgs.map(link => {
        photos.push({ src: link, width: 3, height: 3 });
      });
      setItems([...photos]);
      setFileCnt(_imgs.length);
    }
  }, []);
  ...
  return (
    <Formik
      initialValues={{
        id: uuidv4(),
        userId: user.id,
        difficulty: classInstance.difficulty,
        category: classInstance.category,
        atAGlance: classInstance.atAGlance,
        equipment: classInstance.equipment,
        images: classInstance.images,
      ...
      onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
        try {
          ...
          await dispatch(updateClass(classInstance.id, values));
      ...
      {({
        ...
        >
          ...
          <Box mt={2} display="flex" justifyContent="flex-start">
            <SubmitButton text="Save class" isSubmitting={isSubmitting} />
          </Box>
        </form>
      )}
    </Formik>
